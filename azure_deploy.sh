...
}" "$COSMOS_DB_ACCOUNT_NAME"> depl.json
}
...
sql_cosmos_depl(){
    generate_sql_cosmos
	az deployment group create -g "$RESOURCE_GROUP" --template-file depl.json
...
db_deploy(){
    ...
    if [ "$DATABASE_TYPE" = "SQL" ]; then
        ...
        az functionapp config appsettings set --name "$APP_NAME" --resource-group "$RESOURCE_GROUP"  "$DB_URI_VAR_NAME=https://$COSMOS_DB_ACCOUNT_NAME.documents.azure.com:443/"
...
redis_deploy(){
    ...
    az functionapp config appsettings set --name "$APP_NAME" --resource-group "$RESOURCE_GROUP" --settings "$REDIS_HOSTNAME_VAR_NAME=https://$REDIS_NAME.redis.cache.windows.net/"
